...
void matmulSchleifenvertauschjki(double * input1,double * input2,double * output){
...
void matmulijkWTemporary(double * input1,double * input2,double * output){
        for(int i=0;i<SIZE;i++){ //kij
                for(int j=0;j<SIZE;j++){
                        double d=0;
                for(int k=0;k<SIZE;k++){
...
void matmulkji(double * input1,double * input2,double * output){
        for(int k=0;k<SIZE;k++){ 
...
int main(int argc, char *argv[]){
    if(argc!=3){printf("less args");printf("%d",argc);return 1;};
    char *l =argv[1];
    char *d =argv[2];
    loopsize=atoi(d);
    SIZE=atoi(l);
    ...
    //long long durationSecs=neumillis-millis;
    ...
    free(input2);
    free(output);
    //long long GFLOPS=(SIZE*SIZE*(2*SIZE-1)*(long long)LOOPCOUNT)/(neumillis-millis);
